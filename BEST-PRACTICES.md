...
> ⑤ Claude Code 的 `/dev` 内部会持续做本地 gate，并在 L/XL 收口时进入 ⑦`/review-all`；Codex 中对应 `$team-command-dev`。
...
```
/plan 做一个面向中小团队的 AI 知识库，能上传文档、问答
```

它会：先问几个产品级问题（目标用户/核心价值/范围边界）→ 把产品拆成功能模块 → 写到根目录 `roadmap.md`（每个模块带优先级、复杂度、依赖、建议顺序，并标出 MVP 最小集）。**只规划不写代码**，规划完你挑着做。
...
| **Phase 2 迭代开发** | TDD 写代码 → 自动 `/check` → 更新当前 task/module → 必要时进入 `/review-all` | 看进度 |
| **Phase 3 验收** | 跑测试 + 类型检查 + lint + git 状态 + 产品验收说明 | —— |
| **Phase 4 记录事件** | 追加最终 events.jsonl；metrics 由 events 聚合，触发警告时提醒 | —— |
...
8 道关卡：集成验证 → 安全审计 → 性能 → 无障碍 → 配置 → Git 规范 → 发布决策 → 部署。**还会强制确认回滚方案和 known risk 处置**（accept / mitigate / defer）；高风险发布必须给 Owner Decision Brief。M 级以上必须过 `/ship` 才发布。
...
| 审查打回 | 按 review report 委托修复，仍不过列问题 | 决定继续修/请求 Owner 决策/暂缓合并 |
...
```
/dev 完成 → 写 events.jsonl ─┐
                              ├─► /standup 聚合 metrics → 诊断瓶颈 → 你调整 → 下次更准
会话结束 → 写 journal.md ───┘
```
