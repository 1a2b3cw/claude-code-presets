...
- 当 spec 声明 `Owner Decision Required：yes` 且存在 Owner Decision Contract 时，先运行 `decision validate <brief>`，并在 preflight/transition 传入同一 `--decision <brief>`。未确认、错模块或错类型必须停止；不得因为拥有工具权限而自行把 Brief 标为 confirmed。
- 存在 `.claude/workspace/planning/change-impact-contract.md` 且请求来自体验反馈或既有行为变更时，先读取并运行 `change validate <brief>`；仅在 Brief 通过后按其中的同步项实施，必要时将它传给 `delivery preflight --change <brief>`。
...
- 修改完成后更新 Change Impact Brief 的同步项和验证证据；发现 Brief 漏掉产品、架构、安全或运行影响时停止扩大实现，交回 Architect-Planner/Owner。

## 代码规范
- **语言/框架的具体规范一律遵循 `.agents/rules/`**（TypeScript、Python 等各自的规则文件），不在此重复
- 遵循 spec，不擅自添加功能，不处理不可能的场景
- 单一职责：一个文件/函数只做一件事
- 用具体的错误类型，错误处理放在边界层，内部抛异常
- 不引入安全漏洞（注入、跨站脚本、硬编码密钥等；hooks 会自动拦截严重项）
- 前端 UI 任务：交互元素加无障碍标签（见 `.agents/rules/design.md`）

## UI 设计要求
- 实现 UI 前，先读 `.agents/rules/design.md` 和项目 spec 中的设计方向
